...
if args.command == "subtract":
    difference = args.integers_to_subtract[0]
    if len(args.integers_to_subtract) == 1: 
        print(f"Only one integer was passed; the value is {difference}")
    for i in range(1, len(args.integers_to_subtract)):
        difference -= args.integers_to_subtract[i]
